-- Atomic seed import function.
-- The calling PostgreSQL statement is one transaction; any raised exception
-- rolls back all writes performed by this function.
...
begin
  ...
  select * from jsonb_populate_recordset(null::public.products, payload->'products')
  on conflict (id) do update set
    slug = excluded.slug,
    name = excluded.name,
